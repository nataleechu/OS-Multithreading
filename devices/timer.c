...
void timer_sleep (int64_t ticks)
{
  ...
  // inserts in order by timer
  list_insert_ordered(&blocked_list, &t->blockedelem, set_sorted_timer, NULL);
  sema_down(&t->s);
  intr_set_level (old_level);
...
static void timer_interrupt (struct intr_frame *args UNUSED)
{
  ...
  // Ben driving
  // wakes up threads who have slept for allotted amount of time
  e = list_begin(&blocked_list);
  bool over = false;
  while (e != list_end(&blocked_list) && !over) {
